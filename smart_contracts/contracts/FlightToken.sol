 ...
 */
contract FlightToken is IERC20 {
  ...
  */
  ...
  function paymentHandlerCancelled(address cust, uint price) public isOwner() returns (uint, uint) {
    require(cust != address(0));
    transfer(cust, price);

    emit PaymentRefunded(msg.sender, cust, price);

    return (balanceOf(msg.sender), balanceOf(cust));
  }


  // handle refund if delayed
  function paymentHandlerDelayed(address cust, uint dist) public isOwner() returns (uint, uint) {
